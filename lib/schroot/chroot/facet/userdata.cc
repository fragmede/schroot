    ...
    namespace facet
    {
      ...
      namespace
      {
        ...
        validate_keyname(const std::string& key)
        {
          ...
          // underscores.
          static schroot::regex permitted("^([a-z][a-z0-9]*\\.)+[a-z][a-z0-9-]*$");

          // These would permit clashes with existing setup environment
          // variables, and potentially security issues if they were
          // user-settable.
          static schroot::regex reserved("^(((auth|chroot|host|libexec|mount|session|status|sysconf)\\..*)|setup.data.dir)$");
          ...
          for (const auto& elem : unused)
            {
              ...
              try
              ...
              catch (const std::runtime_error& e)
                {
                  ...
                  try
                    {
                      const schroot::error_base& r =
                        dynamic_cast<schroot::error_base const&>(e);
