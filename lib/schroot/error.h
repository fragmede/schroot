...
#ifndef SCHROOT_ERROR_H
#define SCHROOT_ERROR_H
  ...
  protected:
    ...
    struct add_reason_helper<A, true>
    {
                        ...
                        A const&     value)
      {
        try
          {
            const schroot::error_base& eb(dynamic_cast<schroot::error_base const&>(value));
            if (!reason.empty())
              reason += '\n';
            reason += eb.why();
          }
...
#include <schroot/error.tcc>

#endif /* SCHROOT_ERROR_H */
